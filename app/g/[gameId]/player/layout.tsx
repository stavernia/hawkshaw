...
export default async function ScopedPlayerLayout({
  ...
  return (
    <div className="mx-auto flex min-h-screen w-full max-w-5xl flex-col gap-8 px-6 py-6 md:px-10">
      <header className="app-surface flex items-center justify-between rounded-[1.75rem] border border-white/70 px-5 py-4">
        <div>
          ...
          <h1 className="font-[family-name:var(--font-heading)] text-3xl">Player Area</h1>
        </div>
        <div className="flex items-center gap-3">
          <Link className="text-sm text-muted-foreground transition hover:text-foreground" href={SITE_ROUTES.gameHost(gameId)}>
            Host area
          </Link>
